...
| 수수료 | 없음 |
| 최소 환전 단위 | 1포인트 (추후 설정 가능하게 확장) |
| 최대 환전 한도 | 미정 (추후 한도 설정 기능 추가 가능) |
...
- [ ] 환전 최소/최대 한도 정책 결정 필요 (Reus 확인)
- [ ] 오피스랜드 포인트 기능 구현 일정 확정 필요 (Garden)
- [ ] 실패 알림 UX — 유저에게 어떻게 실패를 노티할지 (토스트? 모달?)
- [ ] 환전 내역 보존 기간 정책 필요
- [ ] 어드민 환율 관리 UI 별도 구현 여부 (MVP엔 DB 직접 수정도 가능)
